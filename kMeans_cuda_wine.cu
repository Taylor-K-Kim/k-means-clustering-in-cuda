...
// Date: 12/08/2023
//
// NOTE: THIS IS FOR WINE QUALITY DATASET.
//
// FOR CLUSTERING, from the original dataset, the first row with data title are removed
// and the last column for target (quality) is removed.
// At the end of the program, the resulting clusters are added to the last
// column of the original dataset to check for correctness.
...
#define N 1599   // number of data points
#define F 11     // number of features in dataset
#define K 6     // number of clusters and centroids
#define T 256   // threads per block, ensures enough block
#define MAX_ITER 100    // instead of checking for convergence, not good
